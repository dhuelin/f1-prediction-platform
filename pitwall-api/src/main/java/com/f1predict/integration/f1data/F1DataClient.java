package com.f1predict.integration.f1data;
...
/**
 * HTTP client for f1-data-service — the one remaining cross-process call.
 *
 * The prediction and scoring domains each carried their own F1DataClient with an
 * identical getQualifyingDeadline implementation. Sharing a Spring context made
 * the duplicate class names collide, and the duplication had no reason to exist,
 * so both are replaced by this single client.
 *
 * Every method fails open: f1-data-service being unavailable must not block a
 * prediction submission or abandon a scoring run.
 */
@Component
public class F1DataClient {
     ...
     * or the F1 data service is unavailable (fail open — the caller allows the
     * submission / scores everything).
     */
    public Instant getQualifyingDeadline(String raceId) {
        try {
            ...
            log.warn("F1 data service unavailable for deadline check — failing open. raceId={}", raceId);
            return null;
        }
    }
     ...
     * Returns an empty list when no session is active or the service is unavailable.
     */
